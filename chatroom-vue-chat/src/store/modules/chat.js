...
const chat = {
    state: sessionStorage.getItem('chat') ? JSON.parse(sessionStorage.getItem('chat')) : {
    ...
    actions: {
        async initAllChatData(context) {
            ...
            await getAllGroupMsg().then(resp => {
                if(resp.data !== null){
                    for(let key in resp.data){
                        context.state.curMsgSession.groupMsgMap.set(key,resp.data[key]);
                    }
                }
            ...
            await getAllFriendMsg(user.state.id).then(resp => {
                if(resp.data !== null){
                    for(let key in resp.data){
                        context.state.curMsgSession.friendMsgMap.set(key,resp.data[key]);
                    }
                }
            })
            ...
            //群聊目前只有一个，暂时写死
            context.state.curMsgList = context.state.curMsgSession.groupMsgMap.get("1");
            ...
            //存入sessionStorage
            // window.sessionStorage.setItem("chat",context.state)
        }
    }
