<template>
  <el-container>
    ...
    <el-container>
      <div style="background-color: #4a4d50;width: 299px;">
        <el-aside>

          <el-menu unique-opened router background-color="#4a4d50" text-color="#ffffff">

            <div  v-for="(item, index) in routes" :key="index">
              <!-- 子菜单数量大于1 -->
              <el-submenu :index="item.path" v-if="item.children && item.children.length > 1 && item.hidden !== true">
              ...
              <!-- 子菜单数量等于1 -->
              <el-menu-item :index="item.path" v-if="item.children && item.children.length == 1 && item.hidden !== true">
